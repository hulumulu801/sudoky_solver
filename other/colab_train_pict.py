...
!cp -r /content/gdriver/"My Drive"/data_pict.tar.gz . # копируем архив с изображениями
!tar -xvf data_pict.tar.gz # разархивируем
...
checkpoint = ModelCheckpoint("digit_recognition.hdf5", monitor = "val_acc", save_best_only = True, verbose = 1)
learning_rate_reduction = ReduceLROnPlateau(monitor = "val_acc", patience = 3, verbose = 1, factor = 0.5, min_lr = 0.00001)

history = model.fit_generator(train_generator, steps_per_epoch = nb_train_samples // batch_size, epochs = epochs, validation_data = val_generator, validation_steps = nb_validation_samples // batch_size, callbacks = [checkpoint, learning_rate_reduction])
